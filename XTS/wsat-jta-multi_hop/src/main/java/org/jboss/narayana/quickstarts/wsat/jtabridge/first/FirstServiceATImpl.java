...
public class FirstServiceATImpl implements FirstServiceAT {
    ...
    /**
     * Increment the first counter. This is done by updating the counter within a JTA transaction. The JTA transaction
    ...
    public int getSecondCounter() {
        System.out.println("[SERVICE_1] Calling getCounter on the WS secondClient stub. The registered interceptor will bridge rom JTA to WS-AT");
        return getSecondClient().getCounter();
    }
    ...
    private SecondServiceAT getSecondClient() {
        ...
        if (secondClient == null)
        {
            try {
                ...
                throw new RuntimeException("Error creating SecondClient instance", e);
            }
        }
        return secondClient;
    }
